    ...
    def __init__(self, class_name:str, logfile_prefix_name:str=None)->None:
        ...
        if not self._logger.handlers:
            ...
            if self._settings_dict['log_to_file']:
                ...
                # Ensure the log directory exists to avoid FileNotFoundError
                try:
                    log_dir = os.path.dirname(log_file) or self._settings_dict['logs_dir']
                    if log_dir and not os.path.exists(log_dir):
                        os.makedirs(log_dir, exist_ok=True)
                except Exception:
                    # If directory creation fails, fall back to console-only logging
                    self._logger.warning(f"Could not create log directory '{log_dir}'. Logging to file disabled.")
                else:
